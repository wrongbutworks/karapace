...
3. Configure a client-credentials mapping that emits a `roles` claim
   (space-delimited) with the Karapace role values your tests need
   (`schema:read`, `schema:write`, `subject:read`, `subject:write`,
   `config_subject:update`, `config_global:update`, `schema:delete`,
   `subject:delete`).
