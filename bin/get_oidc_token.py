...
Fetches an OIDC access token for use against the Schema Registry. Used by
bin/smoke-test-schema-registry.sh and can be run ad-hoc by developers.

The default provider is the local Keycloak container, whose client secret is
looked up via the admin API. Any other OIDC_PROVIDER is treated as a cloud
provider: the token is fetched directly with client_credentials using the
supplied OIDC_TOKEN_URL and OIDC_CLIENT_SECRET.
"""
